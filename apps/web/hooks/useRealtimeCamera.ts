...
interface UseRealtimeCameraOptions {
  enabled: boolean;
...
interface UseRealtimeCameraReturn {
...
function isSessionOpen(state: RealtimeState): boolean {
...
export function useRealtimeCamera({
  enabled,
  sessionState,
  sessionKey,
  sendJson,
  ...
  const sessionOpen = enabled && isSessionOpen(sessionState);
  ...
  const refreshDevices = useCallback(async () => {
    ...
    try {
      ...
      // Ignore device enumeration failures and fall back to generic camera access.
    }
  }, []);
