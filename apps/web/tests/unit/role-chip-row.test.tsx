...
describe("RoleChipRow", () => {
  ...
  it("renders all 6 chips with role labels", () => {
    render(<RoleChipRow activeRole={null} onSelect={() => {}} locale="zh" groupLabel="身份选择" />);
  ...
  it("marks the active chip with aria-checked=true", () => {
    render(<RoleChipRow activeRole="lawyer" onSelect={() => {}} locale="zh" groupLabel="身份选择" />);
  ...
  it("fires onSelect when a chip is clicked", () => {
    ...
    render(<RoleChipRow activeRole={null} onSelect={onSelect} locale="zh" groupLabel="身份选择" />);
  ...
  it("arrow-right moves focus to next chip and fires onSelect", () => {
    const onSelect = vi.fn();
    render(<RoleChipRow activeRole="researcher" onSelect={onSelect} locale="zh" groupLabel="身份选择" />);
  ...
  it("renders English labels when locale=en", () => {
    render(<RoleChipRow activeRole={null} onSelect={() => {}} locale="en" groupLabel="Role selector" />);
    expect(screen.getByRole("radio", { name: "Researcher" })).toBeTruthy();
  });

  it("uses groupLabel as the radiogroup aria-label", () => {
    const { container } = render(
      <RoleChipRow activeRole={null} onSelect={() => {}} locale="zh" groupLabel="身份选择" />,
    );
    const group = container.querySelector('[role="radiogroup"]');
    expect(group?.getAttribute("aria-label")).toBe("身份选择");
  });
});
